    ...
    switch ( command )
    {
        case    'e' :
...

            {
                ...
                if ( g_Subsystem[0] != 0 )
                {
                    _ansc_sprintf(CName, "%s%s", g_Subsystem, CCSP_COMPONENT_ID_RM);
                }
                else
                {
                    _ansc_sprintf(CName, "%s", CCSP_COMPONENT_ID_RM);
                }
...
int main(int argc, char* argv[])
{
    ...
    pComponentName          = CCSP_COMPONENT_NAME_RM;
    ...
    pthread_t thread;
    pthread_create(&thread, NULL, monitorRebootRequest, "monitorRebootRequest");
    
    AnscTrace("Waiting for the input:\n");

    if ( (argc <= 1) || (argc > 1 && strcmp(argv[1], "-c") != 0) ) 
    {
		while (1)
			sleep(30);
    }
