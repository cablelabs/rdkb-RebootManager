...
#define  CCSP_COMPONENT_ID_RM                              "com.cisco.spvtg.ccsp.rm"
#define  CCSP_COMPONENT_NAME_RM                            "com.cisco.spvtg.ccsp.rm"
#define  CCSP_COMPONENT_VERSION_RM                         1
#define  CCSP_COMPONENT_PATH_RM                            "/com/cisco/spvtg/ccsp/rm"
...
COMPONENT_COMMON_RM,  *PCOMPONENT_COMMON_RM;

#define ComponentCommonDmInit(component_com_rm)                                          \
        {                                                                                  \
            AnscZeroMemory(component_com_rm, sizeof(COMPONENT_COMMON_RM));             \
            component_com_rm->Name        = NULL;                                        \
            component_com_rm->Version     = 1;                                           \
            component_com_rm->Author      = NULL;                                        \
            component_com_rm->Health      = CCSP_COMMON_COMPONENT_HEALTH_Red;            \
            component_com_rm->State       = CCSP_COMMON_COMPONENT_STATE_Running;         \
            if(g_iTraceLevel >= CCSP_TRACE_LEVEL_EMERGENCY)                              \
                component_com_rm->LogLevel = (ULONG) g_iTraceLevel;                      \
            component_com_rm->LogEnable   = TRUE;                                        \
            component_com_rm->MemMaxUsage = 0;                                           \
            component_com_rm->MemMinUsage = 0;                                           \
            component_com_rm->MemConsumed = 0;                                           \
        }


#define  ComponentCommonDmClean(component_com_rm)                                        \
         {                                                                                  \
            if ( component_com_rm->Name )                                                \
            {                                                                               \
                AnscFreeMemory(component_com_rm->Name);                                  \
            }                                                                               \
                                                                                            \
            if ( component_com_rm->Author )                                              \
            {                                                                               \
                AnscFreeMemory(component_com_rm->Author);                                \
            }                                                                               \
         }


#define  ComponentCommonDmFree(component_com_rm)                                         \
         {                                                                                  \
            ComponentCommonDmClean(component_com_rm);                                    \
            AnscFreeMemory(component_com_rm);                                            \
         }
