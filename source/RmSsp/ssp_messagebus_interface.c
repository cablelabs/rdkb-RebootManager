    ...
    )
{
    ...
    CcspTraceInfo(("INFO: bus_handle: %d \n", bus_handle));
    ...
    returnStatus = CcspBaseIf_Register_Event(bus_handle, NULL, "systemReadySignal");

    if ( returnStatus != CCSP_Message_Bus_OK )
    {
         CcspTraceError((" !!! CCSP_Message_Bus_Register_Event: systemReadySignal ERROR returnStatus: %d!!!\n", returnStatus));
